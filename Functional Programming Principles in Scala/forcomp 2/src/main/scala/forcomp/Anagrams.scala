...
object Anagrams extends AnagramsInterface {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = (w.toList).groupBy((c: Char) => c.toLower).map{ case(k, v) => (k, v.length) }.toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    ...
    case (acc, (ch,tm)) => {
      acc ++ ( for { comb <- acc; n <- 1 to tm } yield comb ++ List((ch, n)) )
    }
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    (y foldLeft x){
      case (xDiff, yElem) =>
        for (xElem <- xDiff) yield (xElem._1, if(yElem._1 == xElem._1) xElem._2 - yElem._2 else xElem._2)
    }.filter(_._2 > 0)
