...
trait DecoderInstances {
  ...
  implicit val stringDecoder: Decoder[String] = Decoder.fromPartialFunction ({case j: Json.Str =>
    j.value})
  /** A decoder for `Boolean` values */
  // TODO Define an implicit value of type `Decoder[Boolean]`
  implicit val booleanDecoder: Decoder[Boolean] = Decoder.fromPartialFunction({case j: Json.Bool =>
    j.value})
  /**
  ...
  implicit def listDecoder[A](implicit decoder: Decoder[A]): Decoder[List[A]] =
    Decoder.fromPartialFunction({
      case Json.Arr(items) => items.flatMap(item => decoder.decode(item))
    })
  ...
  def field[A](name: String)(implicit decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction({
      case Json.Obj(map) if map.contains(name) => decoder.decode(map.getOrElse(name,Json.Null))
    })
